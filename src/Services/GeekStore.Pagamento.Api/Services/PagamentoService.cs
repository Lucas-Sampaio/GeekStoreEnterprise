...
namespace GeekStore.Pagamentos.Api.Services
{
    public class PagamentoService : IPagamentoService
    {
        ...
        public async Task<ResponseMessage> AutorizarPagamento(Pagamento pagamento)
        {
            ...
            if (transacao.Status != EStatusTransacao.Autorizado)
            {
        ...
        public async Task<ResponseMessage> CancelarPagamento(Guid pedidoId)
        {
            ...
            var transacaoAutorizada = transacoes?.FirstOrDefault(t => t.Status == EStatusTransacao.Autorizado);
            var validationResult = new ValidationResult();
            ...
            if (transacao.Status != EStatusTransacao.Cancelado)
            {
                validationResult.Errors.Add(new ValidationFailure("Pagamento",
